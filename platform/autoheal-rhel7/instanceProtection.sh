#!/bin/bash
set -e

echo --------------------------------------------------
echo Instance Protection started `date`
echo --------------------------------------------------
echo

MTOKEN=$(curl -s -X PUT "http://169.254.169.254/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 21600")
INSTANCE_ID=$(curl --silent --max-time 3 -H "X-aws-ec2-metadata-token: $MTOKEN" http://169.254.169.254/latest/meta-data/instance-id)
ASG_NAME=$(/usr/local/bin/aws ec2 describe-tags --filters "Name=resource-id,Values=${INSTANCE_ID}" "Name=key,Values=aws:autoscaling:groupName" --query 'Tags[0].Value' --output text)

echo --------------------------------------------------
echo 1. Enable Instance Protection
echo --------------------------------------------------

/usr/local/bin/aws autoscaling set-instance-protection \
--instance-ids $INSTANCE_ID \
--auto-scaling-group-name $ASG_NAME \
--protected-from-scale-in
exit_code=$?

if [ $exit_code -eq 0 ]; then
	echo "Enabled Instance Protection"
else
	echo "Failed Enabling Instance Protection"
    exit $exit_code 
fi

echo --------------------------------------------------
echo 2. Disable Instance Protection
echo --------------------------------------------------

/usr/local/bin/aws autoscaling set-instance-protection \
--instance-ids $INSTANCE_ID \
--auto-scaling-group-name $ASG_NAME \
--no-protected-from-scale-in
exit_code=$?

if [ $exit_code -eq 0 ]; then
	echo "Disabled Instance Protection"
else
	echo "Failed Disabling Instance Protection"
    exit $exit_code 
fi

exit $exit_code