...
require_relative "builders/launch_template_builder"
...
require_relative "builders/scaling_policy_builder"
...
# @attr_reader autoscaling_group [String] name of autoscaling group resource
class AwsAutoscale < Consumable
  ...
  def initialize(component_name, definition)
    ...
    (definition["Configuration"] || {}).each do |name, resource|
      ...
      when "AWS::EC2::LaunchTemplate"
        raise "This component does not support multiple #{type} resources" unless @launch_template.empty?

        @launch_template[name] = resource
      ...
      when "AWS::AutoScaling::ScalingPolicy"
        @scaling_policies[name] = resource
    ...
    # Must specify either (XOR) launch_configuration or launch)template
    if !(@launch_configuration.empty? ^ @launch_template.empty?) then
      raise "Should use either LaunchConfiguration or LaunchTemplate resources in component: #{component_name}."
    end
    if @autoscaling_group.empty?
      raise "Must specify AWS::AutoScaling::AutoScalingGroup resource for component: #{component_name}"
    end
    ...
    if @launch_template.empty?
      @launch_data = @launch_configuration
      @image_id = JsonTools.get(@launch_configuration.values.first, "Properties.ImageId", nil)
      @intance_type = JsonTools.get(@launch_configuration.values.first, "Properties.InstanceType", "m3.medium")
    else
      @launch_data = @launch_template
      @image_id = JsonTools.get(@launch_template.values.first, "Properties.LaunchTemplateData.ImageId", nil)
      @intance_type = JsonTools.get(@launch_template.values.first, "Properties.LaunchTemplateData.InstanceType", "m3.medium")
    end
  end
  ...
  def security_rules
    ...
    if @load_balancer.any?
      security_rules += _parse_security_rules(
        type: :ip,
        rules: @load_balancer.values[0]["Security"],
    ...
    security_rules += _parse_security_rules(
      type: :ip,
      rules: @autoscaling_group.values[0]["Security"],
      destination: "#{@component_name}.AsgSecurityGroup",
    )
    ...
    # Autoscaling Group permissions - Instance Protection and Desired Capacity
    security_rules << IamSecurityRule.new(
      roles: "#{@component_name}.InstanceRole",
      actions: %w(
    ...
    # Autoscaling Group permissions - Scheduled Actions
    security_rules << IamSecurityRule.new(
      roles: "#{@component_name}.InstanceRole",
  ...
  def deploy
    ...
    if @bake_instance.any?
      ...
      snapshot_copy = JsonTools.get(@launch_data.values.first, "Properties.Pipeline::CopySourceImage", true)
    ...
    _build_full_stack(platform: soe_ami_details[:platform])
  ...
  def teardown
    ...
    # Scale down the ASG before teardown to give terminating lifecycle hooks a chance to fire and complete
    begin
      ...
      if autoscaling_group_name.nil?
        ...
        Log.debug "Removing instance protection prior to scaling down the ASG"
        ...
        Log.debug "Waiting for #{@component_name.inspect} ASG #{@autoscaling_group_name.inspect} to scale down before teardown (max 30 minutes)"
      ...
      Log.warn "Failed to scale down ASG #{@autoscaling_group_name.inspect} before teardown - #{e}"
    end
  ...
  def _build_bake_stack(soe_ami_id: nil, soe_ami_name: nil, platform: nil, soe_tags: nil)
    ...
    begin
      ...
      raise AutoScaleBakeError.new("Failed to create instance bake stack - #{e}", e)
    ensure
  ...
  # @param platform [Symbol] Operating system platform
  def _build_full_stack(platform: nil)
    Log.info "Creating autoscale consumable resources"
    ...
    # get autoscale group name, poll for scaling activities
    ...
    # create scaling activity poll thread, method must handle exceptions
    asg_activities_thread = _create_scaling_poll_thread(autoscaling_group_name: autoscaling_group_name)

    # update autoscale group, it waits until group reaches desired amount of instancies
    ...
    begin
      ...
      # cleanup scaling activity poll thread, method must handle exceptions
      _cleanup_scaling_poll_thread(asg_activities_thread: asg_activities_thread)
  ...
  # cleans up scaling activities thread
  # @param autoscaling_group_name [String] autoscale group name
  def _cleanup_scaling_poll_thread(asg_activities_thread:)
    if !asg_activities_thread.nil?
      begin
        asg_activities_thread_id = asg_activities_thread.object_id

        Log.debug "Stopping autoscale polling thread: #{asg_activities_thread_id}"

        # block until the thread dies - Thread.kill(..).join
        # we need to wait until the thread dies, there can be a delay here
        # seems to surface on RHEL6, Mac and CentOS containers exit immediately
        # can also be related to different Ruby version, 2.4/2.5
        # https://stackoverflow.com/questions/49490278/wait-for-a-thread-to-die-in-ruby

        # related test failing under CI
        #   - aws_autoscale.rb, _cleanup_scaling_poll_thread.executes
        Thread.kill(asg_activities_thread).join
        Log.debug "Stopped autoscale polling thread: #{asg_activities_thread_id}"
      rescue => e
        Log.debug "Failed to stop autoscale polling thread: #{asg_activities_thread_id}, error: #{e}"
      end
    else
      Log.debug "Skipping clean up, asg_activities_thread is nil"
    end
  end

  # creates and returns a thread to poll scling activities
  # @param autoscaling_group_name [String] autoscale group name
  def _create_scaling_poll_thread(autoscaling_group_name:)
    result = nil

    if !autoscaling_group_name.nil?
      begin
        Log.debug "ASG: polling autoscale activities, #{autoscaling_group_name}"
        _trace_asg_url(autoscaling_group_name: autoscaling_group_name)

        Log.debug "Polling autoscale activities, group: #{autoscaling_group_name}"
        result = _poll_scaling_activities_async(
          autoscaling_group_name: autoscaling_group_name
        )
      rescue => e
        Log.debug "Failed to poll scaling activities, group: #{autoscaling_group_name} - #{e}"
      end
    else
      Log.warn "Cannot poll autoscale activities, group is null or empty"
    end

    result
  end

  # returns AWS Console autoscale group url
  # @param autoscaling_group_name [String] autoscale group name
  def _get_asg_url(autoscaling_group_name:)
    region = Defaults.default_region

    autoscaling_group_url = [
      "https://#{region}.console.aws.amazon.com/ec2/autoscaling/home?region=#{region}#",
      "AutoScalingGroups:id=#{autoscaling_group_name};view=history"
    ].join('')

    autoscaling_group_url
  end

  # outputs AWS Console autoscale group url
  # @param autoscaling_group_name [String] autoscale group name
  def _trace_asg_url(autoscaling_group_name:)
    autoscaling_group_url = _get_asg_url(
      autoscaling_group_name: autoscaling_group_name
    )

    Log.output [
      "ASG AWS Console URL, #{autoscaling_group_name}",
      autoscaling_group_url
    ].join("\n")
  end

  # polls autoscaling activities in async mode
  # @param autoscaling_group_name [String] autoscale group name
  # @param poll_frequency [Integer] poll frequency in seconds
  def _poll_scaling_activities_async(autoscaling_group_name:, poll_frequency: 90)
    Thread.new {
  ...
  # polls autoscaling activities in sync mode
  # @param autoscaling_group_name [String] autoscale group name
  # @param poll_frequency [Integer] poll frequency in seconds
  # @param max_attempts [Integer] max attempts to perform before exiting the poll cycle
  def _poll_scaling_activities(autoscaling_group_name:, poll_frequency: 90, max_attempts: 120)
    poll_attempt = 1

    while true do
      Log.info "ASG group: #{autoscaling_group_name}, polling scaling activities, attempt #{poll_attempt}/#{max_attempts}"

      begin
        scaling_activities = AwsHelper.describe_scaling_activities(
          autoscaling_group_name: autoscaling_group_name
        )

        _format_scaling_activities(autoscaling_group_name: autoscaling_group_name, scaling_activities: scaling_activities)
      rescue => e
        Log.error "ASG group: #{autoscaling_group_name}, attempt #{poll_attempt}/#{max_attempts}, failed to poll scaling activities - #{e}"
      end

      Log.debug "ASG group: #{autoscaling_group_name}, attempt #{poll_attempt}/#{max_attempts}, sleeping #{poll_frequency} sec before the next poll"
      sleep(poll_frequency)

      poll_attempt = poll_attempt + 1

      if !max_attempts.nil?
        if poll_attempt > max_attempts
          Log.debug "ASG group: #{autoscaling_group_name}, reached max attempts - #{poll_attempt}/#{max_attempts}], returning"
          break
        else
          Log.debug "ASG group: #{autoscaling_group_name}, attempt - #{poll_attempt}/#{max_attempts}, continue"
        end
      end
    end
  end

  # formats and outputs autoscaling activities in table mode
  # @param scaling_activities [Object] result of AwsHelper.describe_scaling_activities() call
  def _format_scaling_activities(autoscaling_group_name:, scaling_activities:)
    Log.info "ASG group: #{autoscaling_group_name}, printing activity history"

    current_item_index = 1
    item_count = scaling_activities.activities.count

    scaling_activities.activities.each do |event|
      messages = []
      ...
      messages << [
        ...
        event.description,
        event.start_time,
        event.end_time == nil ? '' : event.end_time
      ]

      Log.info "ASG group: #{autoscaling_group_name}, printing activity item #{current_item_index}/#{item_count} - #{event.activity_id}"
      # output a nice table for scaling event
      table_data = ObjToText.to_string_table messages
      table_data.each do |table_row|
        Log.info table_row
      end

      # exapand a detailed message as a debug string for scaling event
      Log.info "ASG group: #{autoscaling_group_name}, Activity Cause Details - #{event.activity_id}: #{event.cause}"

      current_item_index = current_item_index + 1
    end

    Log.info "ASG group: #{autoscaling_group_name}, printing activity history completed!"
  end
  ...
  def _bake_instance_template(
    ...
    windows_ou = _resolve_default_ou_path(
      current_value: JsonTools.get(@bake_instance.values.first,
    ...
    _process_instance(
      ...
      default_instance_type: @intance_type,
  ...
  def _full_template(
    ...
    windows_ou = _resolve_default_ou_path(
      current_value: JsonTools.get(@launch_data.values.first,
                                   "Properties.Pipeline::WindowsOUPath",
                                   "@default"),
      ams: Defaults.sections[:ams],
      qda: Defaults.sections[:qda],
    ...
    user_data_variables = {
      ...
      "DeploymentEnv" => Defaults.sections[:env] == "prod" ? "Production" : "NonProduction",
      "Domain" => Defaults.ad_join_domain,
      "MetadataResource" => (@launch_configuration.empty?) ? @launch_template_name : @launch_configuration_name,
      "NoProxy" => Context.environment.variable("aws_no_proxy", ""),
      "Region" => { "Ref" => "AWS::Region" },
    ...
    if !@launch_configuration.empty?
    ...
    elsif !@launch_template.empty?
      _process_launch_template_configuration(
        template: template,
        launch_template_definition: @launch_template,
        image_id: image_id,
        platform: platform,
        user_data: user_data,
        instance_profile: { "Ref" => "InstanceProfile" },
      ...
      raise "Both Launch Configuration and Launch Template are missing unable to proceed"
    ...
    # Generate ScalingPolicy resources
    ...
    @alarms.each do |name, definition|
      namespace = JsonTools.get(definition, "Properties.Namespace", "AWS/EC2")
      definition["Properties"]["Namespace"] = namespace

      if namespace == "AWS/EC2"
        # Only supply defaults in the case that namespace is AWS/EC2 - anything else will be custom set by the user
        definition["Properties"]["ActionsEnabled"] = true
        definition["Properties"]["Dimensions"] = JsonTools.get(definition, "Properties.Dimensions", [{ "Name" => "AutoScalingGroupName", "Value" => { "Ref" => @autoscaling_group_name } }])
        definition["Properties"]["EvaluationPeriods"] = JsonTools.get(definition, "Properties.EvaluationPeriods", 2)
        definition["Properties"]["MetricName"] = JsonTools.get(definition, "Properties.MetricName", "CPUUtilization")
        definition["Properties"]["Statistic"] = JsonTools.get(definition, "Properties.Statistic", "Average")
        definition["Properties"]["Period"] = JsonTools.get(definition, "Properties.Period", 60)
      end
    end
    _process_alarms(
      template: template,
