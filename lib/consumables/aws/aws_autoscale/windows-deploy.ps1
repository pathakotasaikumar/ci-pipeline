...
Log-InfoMessage "cfn-get-metadata --region $Region --stack $StackId --resource $ResourceToSignal --key WAIT_CONDITION"
$WAIT_CONDITION_RESOURCE=(cfn-get-metadata --region $Region --stack $StackId --resource $ResourceToSignal --key WAIT_CONDITION)

$RemoteLogsPath = "<| RemoteLogsPath |>/$WAIT_CONDITION_RESOURCE/$InstanceId"
...
try {
  ...
  Process-DeployStages -Stage $Stage `
                       -ResourceToSignal $WAIT_CONDITION_RESOURCE `
  ...
  Log-InfoMessage "Signalling CloudFormation with cfn-signal (error=1; region=$Region; stack=$StackId; resource=$WAIT_CONDITION_RESOURCE)"
 
  cfn-signal.exe -e 1 --region $Region --stack $StackId --resource $WAIT_CONDITION_RESOURCE
