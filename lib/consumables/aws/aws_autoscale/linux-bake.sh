...
# Write all output to /var/log/bake.log
exec > >(tee /var/log/bake.log | logger -t user-data -s 2>/dev/console) 2>&1
...
function install_cfn_tools_py {
  ...
  validate_exit_code $? "Unable to unzip aws-cfn-bootstrap-latest.zip"
...
# Signal result of cfn-init back to CloudFormation
echo "INFO: Uploading logs and signalling CloudFormation (error=0; region=$REGION; stack=$STACK_ID; resource=$RESOURCE_TO_SIGNAL)"
upload_logs
