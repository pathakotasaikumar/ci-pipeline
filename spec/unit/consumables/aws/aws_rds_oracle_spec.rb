...
require 'aws_rds'
require 'aws_rds_oracle'

RSpec.describe AwsRdsOracle do
  before(:context) do
    @test_data = YAML.load(File.read("#{TEST_DATA_DIR}/db_instance_oracle_builder_spec.yaml"))['UnitTest']

    Context.component.set_variables('Test-Component', {
      'DatabaseArn' => 'MyDatabaseArn'
    })
  end
  ...
  context '.initialize' do
    it 'initialises without error' do
      @test_data['ComponentDefinition']['Valid'].values.each { |definition, index|
        expect { AwsRdsOracle.new(@test_data['ComponentName'], definition) }.not_to raise_error
      }
    end
  end
  ...
  context '.security_items' do
    it 'returns security items' do
      aws_rds = AwsRdsOracle.new(
        @test_data['ComponentName'],
        @test_data['ComponentDefinition']['Valid']['OracleFullBuild']
      )
      expect(aws_rds.security_items).to eq @test_data["FullBuildOutputs"]['SecurityItems']
    end
  end
  ...
  context '.security_rules' do
    it 'returns security rules for component definition with full db' do
      ...
      expect(aws_rds.security_rules.to_yaml).to eql @test_data["FullBuildOutputs"]["SecurityRules"].to_yaml()
    end
  end

  context '.deploy' do
    it 'successfully executes' do
      aws_rds = AwsRdsOracle.new(@test_data['ComponentName'], @test_data['ComponentDefinition']['Valid']['OracleFullBuild'])
