...
RSpec.describe LambdaFunctionBuilder do
  before(:context) do
    @dummy_class = DummyClass.new
    @dummy_class.extend(LambdaFunctionBuilder)
    @test_data = YAML.load(
      File.read("#{TEST_DATA_DIR}/#{File.basename(__FILE__, ".*")}.yaml")
    )
  end

  context '._process_lambda_function' do
    it 'successfully builds template' do
      @test_data['UnitTest']['Input']['_process_lambda_function'].each_with_index do |definition, index|
        template = { "Resources" => {}, "Outputs" => {} }
        expect {
          @dummy_class._process_lambda_function(
            template: template,
            function_definition: definition
          )
        }.not_to raise_error
        expect(template).to eq @test_data['UnitTest']['Output']['_process_lambda_function'][index]
      end
    end
  end

  context '._process_event_source_mapping' do
    it 'successfully builds template' do
      @test_data['UnitTest']['Input']['_process_event_source_mapping'].each_with_index do |definition, index|
        template = { "Resources" => {}, "Outputs" => {} }
        expect {
          @dummy_class._process_event_source_mapping(
            template: template,
            function_name: 'Function',
            definitions: definition
          )
        }.not_to raise_error
        expect(template).to eq @test_data['UnitTest']['Output']['_process_event_source_mapping'][index]
      end
    end
  end
