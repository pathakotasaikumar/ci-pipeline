...
require 'builders/pipeline_db_instance_backup_policy_builder'
require 'builders/security_rule_builder'
require 'json'

RSpec.describe PipelineDbInstanceBackupPolicyBuilder do
  before(:context) do
    ...
    @dummy_class.extend(PipelineDbInstanceBackupPolicyBuilder)
  end

  context '_parse_db_instance_backup_policy' do
    (0..0).each do |index|
      it "returns a list of db_instance backup policies - #{index}" do
        ...
        backup_policy = @dummy_class._parse_db_instance_backup_policy(
          component_name: 'db_instance',
          definitions: @test_data['Input']['_parse_db_instance_backup_policy']['Valid'][index],
          resource_id: 'a1234567890'
        )
        expect(backup_policy.to_hash).to eq(@test_data['Output']['_parse_db_instance_backup_policy']['Valid'][index].to_hash)
      end
    end
  end
end # RSpec.describe
