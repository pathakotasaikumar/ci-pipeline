...
require 'builders/pipeline_dynamodb_backup_policy_builder'
require 'builders/security_rule_builder'
require 'json'

RSpec.describe PipelineDynamoDBBackupPolicyBuilder do
  before(:context) do
    ...
    @dummy_class.extend(PipelineDynamoDBBackupPolicyBuilder)
  end

  context '_parse_dynamodb_backup_policy' do
    (0..0).each do |index|
      it "returns a list of dynamodb backup policies - #{index}" do
        ...
        backup_policy = @dummy_class._parse_dynamodb_backup_policy(
          component_name: 'dynamodb',
          definitions: @test_data['Input']['_parse_dynamodb_backup_policy']['Valid'][index],
          resource_id: 'vol-01234567890'
        )

        expect(backup_policy.to_hash).to eq(@test_data['Output']['_parse_dynamodb_backup_policy']['Valid'][index].to_hash)
      end
    end
  end
end # RSpec.describe
